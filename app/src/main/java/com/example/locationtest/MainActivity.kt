                ...
                if (lapNo <= maxLaps) {
                    ...
                    if (wpt.waypointIndex != lastWptIndex && wpt.waypointIndex != -1) {
                        ...
                        if (lapNo>0) {
                            ...
                            if (time < bestTimes[lastWptIndex] || bestTimes[lastWptIndex] == 0L)
                                bestTimes[lastWptIndex] = time
                            if (timediff <= 0) {
                                tvTimediff.setTextColor(getColor(R.color.green))
                            } else {
                                tvTimediff.setTextColor(getColor(R.color.red))
                            }
                            if (lapNo == 1 || (lapNo == 2 && wpt.waypointIndex == 0)) {
                                tvTimediff.text = ""
                            } else {
                                tvTimediff.text =
                                    String.format("%.2f", (timediff.toDouble() / 1000))
                            }
                        }
                    }
                    if (lapNo==0) {
                        tvRoundDescription.text=getString(R.string.prestart)
                    }
                }
            }
        }
